...
function shuffleDeck() {
  ...
  let tmp = [...cardValues, ...cardValues, ...cardValues, ...cardValues];
  deck = [];
...
function cardSelected(event) {
  //alert(event.target.dataset.value); console.log(event.target.dataset.value)}
  ...
  if (currentCard != 0) {
    ...
    if (currentCard.dataset.value === event.target.dataset.value) {
      ...
      message.textContent = `Congratulations! ${(playerHighlight.textContent =
        currentPlayer.name)}, please go again!`;

      currentCard = 0;
      return;
    } else {
      // Step 6e - Provide a fail message to the player
      message.textContent = "Oh, so sorry!!! Better luck next time!";
      ...
      setTimeout(function() {
        ...
        // (use string interpolation to show which player you're addressing)
        message.textContent = ` ${(playerHighlight.textContent =
    ...
    // (use string interpolation to show which player you're addressing)
    message.textContent = `${(playerHighlight.textContent =
      currentPlayer.name)}, please select another card`;
  }
}
...
function updateScores() {
  score.textContent = `${players[0].name}'s Score: ${players[0].score} | ${
    players[1].name
  }'s Score: ${players[1].score}`;
}
